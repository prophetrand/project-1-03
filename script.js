$(document).ready(function(){
  ...
  var marker;
  // The variable finder is declared in global scope so that once its value is changed by the AJAX function, it can be passed to the Last.fm API.
  var finder;
  ...
  function placeMarker(location, map) {
    ...
    marker.addListener("click", () => {
      // The setMap() method called below removes the marker when the marker is clicked.
      marker.setMap(null);
      // Setting the marker equal to null also prevents it from interfering with the search after being removed.
      marker = null;
    });
  ...
  $("#search-go").on("click", function(){
    ...
    } else if (marker == null && $('#country').val().trim()){
      finder = $('#country').val().trim();
      $("#result-country").text("Results for: " + finder);
      $("#dump-viewer").empty();
      $("#place-check").empty();
      $.ajax({
